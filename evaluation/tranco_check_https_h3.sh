...
# Script to check HTTPS DNS records and H3 support for the top-5000 domains using `dig domain HTTPS +short`
# Usage: ./check_https_h3.sh

CSV_FILE="tranco_5000.csv"
# Generate timestamp in DDMMMYYYY_HHMM format
TIMESTAMP=$(date +"%d%b%Y_%H%M" | tr '[:lower:]' '[:upper:]')
OUTPUT_FILE="https_h3_results_${TIMESTAMP}.txt"
LOG_FILE="https_h3_scan_${TIMESTAMP}.log"
...
echo "HTTPS and H3 Support Analysis" > "$OUTPUT_FILE"
