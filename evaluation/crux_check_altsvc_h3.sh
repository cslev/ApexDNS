...
# Script to check alt-svc headers for H3/QUIC support for Google CrUX top-5000 domains using `curl -I https://domain`
# Usage: ./crux_check_altsvc_h3.sh

CSV_FILE="google_crux_5000.csv"
# Generate timestamp in DDMMMYYYY_HHMM format
TIMESTAMP=$(date +"%d%b%Y_%H%M" | tr '[:lower:]' '[:upper:]')
OUTPUT_FILE="crux_altsvc_h3_results_${TIMESTAMP}.txt"
LOG_FILE="crux_altsvc_h3_scan_${TIMESTAMP}.log"
...
echo "Alt-Svc and H3 Support Analysis (Google CrUX)" > "$OUTPUT_FILE"
